...
public class ProgressService {
	...
	{
		double totalFileNum = processInfo.multiRun_TotalSubNumber*processInfo.multiRun_NumOfRuns;
		double fileNumCount = 0.0;
		for(int s=1;s<=processInfo.multiRun_TotalSubNumber;s++)
		{
			if (processInfo.multiRun_OutPutDir != null && processInfo.multiRun_OutPutDir.trim().length() > 1) {
	...
	public void calculateErrorLimit(ProcessInfo processInfo)
	{
		...
		for(int s=1;s<=processInfo.multiRun_TotalSubNumber;s++)
		{
			String currentLogDir = processInfo.multiRun_OutPutDir+"/"+s+"/log";
			...
			for (int i = 0; i < listOfFiles.length; i++) {
				if (listOfFiles[i].isFile()) {
					...
					if (files.startsWith(fileFilter)) {
						...
						List<String> currentFileContent = DicccolUtilIO.loadFileToArrayList(currentLogDir+"/" + files);
						for(int r=0;r<currentFileContent.size();r++)
							if(currentFileContent.get(r).startsWith("Total Decode Error"))
							{
								double tmpError = Double.valueOf( currentFileContent.get(r).split("\\s+")[4] );
	...
	{
		double totalFileNum = processInfo.ESL_SubEndID-processInfo.ESL_SubStartID+1;
		double fileNumCount = 0.0;
		for(int s=processInfo.ESL_SubStartID;s<=processInfo.ESL_SubEndID;s++)
		{
			if (processInfo.ESL_OutPutDir != null && processInfo.ESL_OutPutDir.trim().length() > 1) {
				File dir = new File(processInfo.ESL_OutPutDir+"/"+processInfo.ESL_SubStartID+"_"+processInfo.ESL_SubEndID+"/"+s+"/OptDicIndex_"+processInfo.ESL_OptDicIndex+"/");
				String[] files = dir.list();
				for (int f = 0; f < files.length; f++) {
					String filename = files[f];
					if ( filename.startsWith("VR_sub_") && filename.endsWith(".txt"))
						fileNumCount++;
				} // for
			} // if
		}
		...
		//execute the bash script
				...
				try {
					String strCmd = "sh /ifshome/dzhu/scripts/Journal_ESL/codeDemo/ESL_Step2.sh "
							...
							+ processInfo.ESL_EpochNum + " "
							+ processInfo.ESL_Lambda;
					System.out.println("CMD: "+strCmd);
					proc = Runtime.getRuntime().exec(strCmd);
				} catch (IOException e1) {
					e1.printStackTrace();
